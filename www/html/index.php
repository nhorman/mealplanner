...
<h1>Meal Planner</h1>  
	...
	function createTables($pdo) {
		...
		if ($meals_exists == FALSE) {
			...
			$result = $pdo->query("CREATE TABLE meals (name VARCHAR(100) NOT NULL, source VARCHAR(100) NOT NULL, famfavorite BOOLEAN NOT NULL, timesmade INT NOT NULL, FOREIGN KEY (source) REFERENCES sources(name), location VARCHAR(512), hide BOOLEAN NOT NULL, PRIMARY KEY (name))");
			if ($result == FALSE) {
